#!/bin/bash
# ─────────────────────────────────────────────────────
#  amalgame-framework-random — Test Runner
#  Usage: ./tests/run_tests.sh [/path/to/amc]
#
#  Stages a fake package cache pointing at the local working tree,
#  pre-builds the per-package facade archive (mimicking what
#  AddCommand.PrecompileFacade does at `amc package add` time),
#  then compiles + links + runs the test sample, asserting the
#  expected [PASS] lines appear on stdout.
#
#  Discovers amc:
...
#    3. `amc` on PATH
#
#  Requires amc 0.7.6+ for the [stdlib].facade pipeline.
# ─────────────────────────────────────────────────────
...
# Resolve to absolute — the runner cd's into a tmp project dir
# before invoking AMC, so relative paths break.
...
# ── Locate amc's runtime/ (core stdlib C headers) ─────
...
# ── Setup ──────────────────────────────────────────────
BUILD_DIR="$(mktemp -d -t afrand-tests-XXXXXX)"
...
echo ""
echo "════════════════════════════════════════════"
echo "  amalgame-framework-random — Test Suite"
echo "════════════════════════════════════════════"
...
PKG_GIT="github.com/amalgame-lang/amalgame-framework-random"
...
[[package]]
name = "amalgame-framework-random"
git  = "$PKG_GIT"
tag  = "$PKG_TAG"
rev  = "$FAKE_SHA"
EOF
...
# ── Compute PlatformTag — mirror PackageRegistry.PlatformTag ──
case "$(uname -s)" in
...
# Note: $PKG_CACHE_DIR is a symlink to $PKG_ROOT — writing under it
# writes to the working tree. We isolate the per-test archive in
# $BUILD_DIR/build/$PLAT and symlink it into place to keep the
# working tree clean during repeated runs.
...
if [ -e "$WORK_BUILD_DIR" ] && [ ! -L "$WORK_BUILD_DIR" ]; then
    echo "WARNING: $WORK_BUILD_DIR exists as a directory; removing." >&2
    rm -rf "$WORK_BUILD_DIR"
fi
rm -f "$WORK_BUILD_DIR"
ln -s "$FACADE_BUILD_DIR" "$WORK_BUILD_DIR"
ARCHIVE="$FACADE_BUILD_DIR/libamalgame-pkg-Random.a"

# ── Pre-build the facade archive (mimics PrecompileFacade) ──
echo "── Pre-compiling facade.am → libamalgame-pkg-Random.a ──"
"$AMC" --lib --quiet "$PKG_ROOT/facade.am" -o "$FACADE_BUILD_DIR/Random-facade"
gcc -O2 -I"$AMC_RUNTIME" -w -c "$FACADE_BUILD_DIR/Random-facade.c" \
    -o "$FACADE_BUILD_DIR/Random-facade.o"
ar rcs "$ARCHIVE" "$FACADE_BUILD_DIR/Random-facade.o"
...
run_test() {
    ...
    cp "$SCRIPT_DIR/stdlib_random.am" "$PROJ_DIR/test.am"
...
run_test "seeded reproducible"          "[PASS] seeded reproducible"
run_test "different seeds differ"       "[PASS] different seeds differ"
run_test "IntRange in bounds"           "[PASS] IntRange in bounds"
run_test "bytes count 32"               "[PASS] bytes count 32"
run_test "bytes in 0..255"              "[PASS] bytes in 0..255"
run_test "float in 0..1"                "[PASS] float in 0..1"
run_test "system bytes count"           "[PASS] system bytes count"
